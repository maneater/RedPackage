...
import com.maneater.ar.camera.open.OpenCameraInterface;
...
public class CameraManager implements Camera.AutoFocusCallback {
    ...
    private AutoFocusListener autoFocusListener = null;

    public void setAutoFocusListener(AutoFocusListener autoFocusListener) {
        this.autoFocusListener = autoFocusListener;
    }

    private boolean isAutoFocusing = false;

    private void requestAutoFocus() {
        if (!isAutoFocusing && camera != null) {
            isAutoFocusing = true;
    ...
    public void onAutoFocus(boolean success, Camera camera) {
        ...
        if (autoFocusListener != null && !autoFocusListener.onAutoFocus(success, camera)) {
            //stop focus
        } else {
            new Handler().postDelayed(focusRunnable, success ? 1000 : 500);
        }
    }
    ...
    public interface AutoFocusListener {
        boolean onAutoFocus(boolean success, Camera camera);
    }
}
